const bookModel = require(`../models/index`).book // load model for table "books"
const Op = require(`sequelize`).Op// load operation from sequelize

// load library "paty" and "filestream"
...
exports.updateBook = async (request, response) => {
    ...
    upload(request, response, async error => {
        ...
        // store selected book ID that will be update
        let id = request.params.id

        // prepare book's data that will be update
        let book = {
            isbn: request.body.isbn,
            title: request.body.title,
            author: request.body.author,
        ...
        // check if file isn't empty, it means update data within reupload file
        if (request.file) {
